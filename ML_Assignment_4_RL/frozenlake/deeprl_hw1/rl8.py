...
def evaluate_policy(env, gamma, policy, max_iterations=int(1e3), tol=1e-3, eval_iter = 0):
    ...
    for iteration in range(max_iterations):
        ...
        o.append((iteration, delta, end - start, eval_iter))
...
def policy_iteration(env, gamma, max_iterations=int(1e3), tol=1e-3):
    ...
    while not stable:
        print(iters)
        start = time.time()
        value_func,iter=evaluate_policy(env,gamma,policy, eval_iter = eval_iters)
        end = time.time()

        diff = end - start

        o.append((iters, iter, diff))

        eval_iters+=iter
        stable,policy=improve_policy(env,gamma,value_func,policy)
        iters+=1

    df = pd.DataFrame(o, columns=['Policy Iterations','Value Iterations', 'Time Diff'])

    df.to_csv('frozenlake8x8stochastic_PI_Prob_0.3.csv')
...
def value_iteration(env, gamma, max_iterations=int(1e3), tol=1e-3):
    ...
    for iteration in range(max_iterations):
        ...
        o.append((iteration, delta, end - start))

        if delta<=tol: break
        value_func_old = value_func_new

    df = pd.DataFrame(o, columns=['Iterations','Delta', 'time'])

    df.to_csv('frozenlake8x8stochastic_VI_prob_0.3.csv')
